...
package com.google.android.apps.miyagi.development.ui.dashboard.support.widget;
...
import static com.google.android.apps.miyagi.development.ui.dashboard.support.widget.ViewUtils.objectEquals;
...
public class AppBarLayout extends LinearLayout {
    ...
    void dispatchOffsetUpdates(int offset) {
        mPrevOffset = mCurrOffset;
        mCurrOffset = offset;
        int dy = mCurrOffset - mPrevOffset;
        if (dy > 0 && mDyOffsetChange < 0 || dy < 0 && mDyOffsetChange > 0) {
            mDyOffsetChange = 0;
        }
        mDyOffsetChange += dy;
    ...
    public static class Behavior extends HeaderBehavior<AppBarLayout> {
        ...
        private static final int OFFSET_THRESHOLD = 40;
        ...
        private void animateOffsetTo(final CoordinatorLayout coordinatorLayout,
            ...
            if (velocity > 0) {
                duration = Math.round(1800 * (distance / velocity));
            } else {
                final float distanceRatio = (float) distance / child.getHeight();
                duration = (int) ((distanceRatio + 1) * 110);
            }
        ...
        private void snapToChildIfNeeded(CoordinatorLayout coordinatorLayout, AppBarLayout abl) {
            ...
            if (offsetChildIndex >= 0) {
                ...
                if ((flags & LayoutParams.FLAG_SNAP) == LayoutParams.FLAG_SNAP) {
                    ...
                    int newOffset;
                    int offsetDirection = abl.mCurrOffset - abl.mPrevOffset;
                    if (abl.getLastFullState() == State.COLLAPSED) {
                        if (offsetDirection < 0 && abl.mDyOffsetChange < -OFFSET_THRESHOLD) {
                            newOffset = snapBottom;
                        } else if (offsetDirection > 0 && abl.mDyOffsetChange > OFFSET_THRESHOLD) {
                            newOffset = snapTop;
                        } else {
                            if (offset > snapBottom + OFFSET_THRESHOLD) {
                                newOffset = snapTop;
                            } else {
                                newOffset = snapBottom;
                            }
                        }
                    } else if (abl.getLastFullState() == State.EXPANDED) {
                        if (offsetDirection < 0 && abl.mDyOffsetChange < -OFFSET_THRESHOLD) {
                            newOffset = snapBottom;
                        } else if (offsetDirection > 0 && abl.mDyOffsetChange > OFFSET_THRESHOLD) {
                            newOffset = snapTop;
